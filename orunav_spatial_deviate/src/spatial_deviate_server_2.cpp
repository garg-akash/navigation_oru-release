...
bool spatialDeviateCB(orunav_msgs::SpatialDeviate::Request &req,
 ...
 //obtain constraints
  ...
  ofstream file;
  string filename = "opt_trajectory_results.m";
  file.open(filename.c_str());
  file << "% Results file from " __FILE__ << endl;
  file << "% Generated " __DATE__ " at " __TIME__ << endl;
  file << endl;
  file << "r_opt = " << "["<<x2_ref[0] << ","<<y2_ref[0]<< ","<<th2_ref[0]<< ","<<p2_ref[0]<<";"<<endl;

  ofstream fvel;
  string fvelname = "opt_control_results.m";
  fvel.open(fvelname.c_str());
  fvel << "% Results file from " __FILE__ << endl;
  fvel << "% Generated " __DATE__ " at " __TIME__ << endl;
  fvel << endl;
  fvel << "u_opt = " << "["<<v2_ref[0] << ","<<omg2_ref[0]<<";"<<endl;
  int N, k;
  ...
  //for (int k = 0; k <= pathIn2.sizePath()/2; ++k) {
    ...
    oru_splt_dev(Px, Py, V_opt, Omg_opt, P_opt, Th_opt, k, pathIn2.sizePath(), N, n_area, discard, del_t, robo_start, x1_ref_or, y1_ref_or, th1_ref_or
                , x2_ref, y2_ref, th2_ref, v2_ref, omg2_ref, l_1, b_1, l_2, b_2, L, d_rear, flag1, ind_ob, ind_or, ind_left
