...
void callback1(ros::NodeHandle &node_handle, const ControllerReportConstPtr& rep1) {
	...
	if (bp_robot1 == 1) {
		...
		if (match(rep1, loaded_path_1)) {
			...
			double goalTime_1 = ros::Time::now().toSec() - bp_robot1_begin; //TODO
			cout<<"[TimeToGoal] for robot1 is : "<<goalTime_1<<" sec"<<endl;
			cout<<ros::Time::now().toSec()<<"sec - "<<bp_robot1_begin<<"sec = "<<goalTime_1<<"sec"<<endl;
...
void callback2(ros::NodeHandle &node_handle, const ControllerReportConstPtr& rep2) {
	...
	if (bp_robot2 == 1) {
		...
		if (match(rep2, loaded_path_2)) {
			...
			double goalTime_2 = ros::Time::now().toSec() - bp_robot2_begin; //TODO
			cout<<"[TimeToGoal] for robot2 is : "<<goalTime_2<<" sec"<<endl;
			cout<<ros::Time::now().toSec()<<"sec - "<<bp_robot2_begin<<"sec = "<<goalTime_2<<"sec"<<endl;
...
void callback3(ros::NodeHandle &node_handle, const ControllerReportConstPtr& rep3) {
	...
	if (bp_robot3 == 1) {
		...
		if (match(rep3, loaded_path_3)) {
			...
			double goalTime_3 = ros::Time::now().toSec() - bp_robot3_begin; //TODO
			cout<<"[TimeToGoal] for robot3 is : "<<goalTime_3<<" sec"<<endl;
			cout<<ros::Time::now().toSec()<<"sec - "<<bp_robot3_begin<<"sec = "<<goalTime_3<<"sec"<<endl;
