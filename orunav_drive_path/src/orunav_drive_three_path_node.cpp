...
int main(int argc, char **argv)
{
  ...
  nh_.param<int>("robot_id3", robot_id_3, 3);
  string file_name_1, file_name_2, file_name_3;
    ...
    cout << "Loaded file 3 sizes : " << loaded_path_3.sizePath() << endl;
    ...
    cout << "First pose file 3: " << loaded_path_3.getPose2d(0) << endl;
    ...
    orunav_generic::savePathTextFile(loaded_path_2,"path_2_off.txt");*/
  	...
  	orunav_msgs::Task task, task_2, task_3;
    ...
    task_3.target.start = orunav_conversions::createPoseSteeringMsgFromState2d(loaded_path_3.getState2d(0));
    task_3.target.goal = orunav_conversions::createPoseSteeringMsgFromState2d(loaded_path_3.getState2d(loaded_path_3.sizePath()-1));
    task_3.target.start_op.operation = task.target.start_op.NO_OPERATION;
    task_3.target.goal_op.operation = task.target.goal_op.NO_OPERATION;
    task_3.target.current_load.status = task.target.current_load.EMPTY;
    task_3.target.goal_load.status = task.target.goal_load.EMPTY;
     
    task_3.target.robot_id = robot_id_3;
    task_3.target.task_id = 1;
    task_3.criticalPoint = -1;  
    task_3.path = orunav_conversions::createPathMsgFromPathInterface(loaded_path_3);
    task_3.update = false;
    task_3.abort = false;
    ...
    ros::ServiceClient client_3 = nh_.serviceClient<orunav_msgs::ExecuteTask>(orunav_generic::getRobotTopicName(robot_id_3,"/execute_task"));
    orunav_msgs::ExecuteTask srv_1, srv_2, srv_3;
    srv_1.request.task = task;
    srv_2.request.task = task_2;
    srv_3.request.task = task_3;
    ...
    if (client_3.call(srv_3)) {
      ROS_INFO("[DrivePathExecutionClientNode] - execute_task sucessfull for 3");
    }
    else
    {
      ROS_ERROR("[DrivePathExecutionClientNode] - Failed to call service: execute_task for 3");
      return 1;
    }    
  ros::spin();
