...
/*std::vector<constraint_extract::PolygonConstraintsVec> getConstraintsVec(constraint_extract::PolygonConstraintsVec &cons, int index) {
...
int main(int argc, char **argv)
{
  ...
  /*std::vector<constraint_extract::PolygonConstraintsVec> constraints_vec = getConstraintsVec(constraints, constraints.size());
  cout<<"size of constraints_vec : "<<constraints_vec.size()<<endl;
  ...
  for (size_t i = 0; i < constraints_vec.size(); i++) {
      double lb_th, ub_th;
      computeThBounds(constraints_vec[i].getThBounds()[0], constraints_vec[i].getThBounds()[1], loaded_path.getPose2d(i)(2), lb_th, ub_th);
      con_th << lb_th << " " << ub_th << endl;
      std::vector<double> A0, A1, b;
      constraints_vec[i].getInnerConstraint().getMatrixFormAsVectors(A0, A1, b);
      size_t size = A0.size();
